...
| 设置 | 深色/浅色/跟随系统（系统栏图标与窗口底色一并跟随）、图库列数、导出文件名后缀、套用预设是否覆盖、随机填充默认种子、导出后是否校验、底栏样式（悬浮／贴底）与底栏入口选择（图片／任务可关，设置固定保留）——每一项都落到实际行为，不做摆设开关 |
...
2. 已知风险（[docs/09](docs/09-风险清单与决策记录.md)）：R-16（缩略图）已修并加了常驻断言；R-17（`GPSVersionID`）
   ...
   两条样本上的写通道偏差逐条钉在 `prodDeviations`／`prodGaps` 里，行为一变就红；R-19 仍未修——读侧会给源文件
   没有的字段补默认值（如「光源 0」），写的时候被整表回写带进文件，校验器又把它算成「匹配」；R-20（深浅色与系统栏
   图标不匹配）已修；R-21 是「底栏入口可关掉」带来的导航死路，已按「至少留一项 + 关掉当前页自动换页」处理。
