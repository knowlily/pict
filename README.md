...
> 当前阶段：**P2 收口 + P3 预设/导出 + 设置页** — P0 骨架、P1 读取链路、P2 编辑引擎（T2.1–T2.13：操作折叠、写通道、时间偏移、GPS 编辑、清空分组、单文件编辑 UI、备份回滚、导出副本）与 P3 的 20 个内置预设、随机填充均已提交；设置页 6 项真正接线（深/浅色主题、图库列数、导出后缀、套用预设覆盖、默认种子、导出后校验）。单测 **529/529 通过**（42 个 XML），金标准 **17/17 通过**（`tools/verify-with-exiftool.sh`，111 条复核断言）
>
...
| 设置 | 深色/浅色/跟随系统（系统栏图标与窗口底色一并跟随）、图库列数、导出文件名后缀、套用预设是否覆盖、随机填充默认种子、导出后是否校验——每一项都落到实际行为，不做摆设开关 |
