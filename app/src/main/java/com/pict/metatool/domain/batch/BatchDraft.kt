 ...
 *
 * @param presetId [BatchMode.PRESET] 与 [BatchMode.RANDOM] 必填
...
data class BatchDraft(
    ...
    val isReady: Boolean
        get() = when (mode) {
            BatchMode.PRESET, BatchMode.RANDOM -> presetId != null
    ...
    fun toPlan(
        ...
        BatchMode.PRESET -> {
            val id = presetId ?: return failureOf(PictError.FIELD_INVALID, "先挑一个预设")
            if (catalog.byId(id) == null) return failureOf(PictError.FIELD_INVALID, "预设不存在：$id")
            successOf(
                plan(
                    listOf(
        ...
        BatchMode.RANDOM -> {
            ...
            val preset = catalog.byId(id) ?: return failureOf(PictError.FIELD_INVALID, "预设不存在：$id")
            if (preset.fields.isEmpty()) return failureOf(PictError.FIELD_INVALID, "预设 $id 没声明任何字段")
            successOf(
                plan(
                    listOf(
